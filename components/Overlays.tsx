  ...
  return (
    <dialog ref={ref} className="modal" onCancel={onClose}>
      <div className="modal-head">
        <div><span className="label label--stamp">Before sending</span><h2>보내기 전에 한 번만 확인해 주세요</h2></div>
        <button className="iconbtn" onClick={onClose} aria-label="닫기"><Icon name="close" /></button>
      </div>
      <p>전화번호, 이메일, API 키 모양은 자동으로 가렸어요. 이름이나 주소처럼 남은 개인정보는 직접 지워 주세요. 아래 내용{images.length > 0 ? `과 사진 ${images.length}장` : ''}이 OpenAI로 전송돼요.</p>
      ...
      {images.length > 0 && (
        <div className="review-images">
          ...
          <p className="attachments__hint">사진은 가려지지 않아요. 카드번호나 이름이 보이면 닫고 지운 뒤 다시 올려 주세요. 사진 속 글자는 서버에서 옮겨 적은 다음 가려요.</p>
        </div>
      )}
      <div className="modal-foot">
        <span>자동으로 가리는 건 완전한 익명화가 아니에요</span>
